...
public class AnnotationTableShared {
    ...
    public AnnotationTableShared(SharedFile file, Integer shift) {
        this.created = false;
        this.file = file;
    ...
    public AnnotationTableShared(SharedFile file) {
        this(file, 0);
    }
    ...
    public AnnotationTableShared create() {
    ...
    public void saveCache() throws Exception {
        if (created) {
            try {
                File cache = new File(file.getFileDirPath() + "/" + cacheName + ".cache");
                PrintWriter fileWriter = new PrintWriter(cache.getAbsoluteFile());
                fileWriter.write(Json.stringify(Json.toJson(this)));
                fileWriter.close();
            } catch (FileNotFoundException fnfe) {
                Logger.of("user." + file.getGroup().getAccount().getUserName()).error("User " + file.getGroup().getAccount().getUserName() +
